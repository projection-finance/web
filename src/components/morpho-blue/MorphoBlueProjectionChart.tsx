...
const MORPHO_INDIGO = "#4F46E5";
const HF_DANGER = "#EF4444";
const SUPPLY_COLOR = "#4F46E5";
...
interface MorphoBlueProjectionChartProps {
  result: MorphoBlueSimulationResult;
  selectedDay: number | null;
  onSelectDay: (day: number) => void;
  chartHeight?: number;
}

export default function MorphoBlueProjectionChart({
  result,
  selectedDay,
  onSelectDay,
  chartHeight = 240,
}: MorphoBlueProjectionChartProps) {
  const [activeTab, setActiveTab] = useState<ChartTab>("netWorth");

  const chartData = useMemo(() => {
    return result.timeline.map((snap: MorphoBlueDaySnapshot) => ({
      day: snap.day,
    ...
    <div>
          ...
          <AreaChart data={chartData} onClick={handleChartClick}>
            <defs>
              <linearGradient id="morpho-nw-grad" x1="0" y1="0" x2="0" y2="1">
                <stop offset="0%" stopColor={MORPHO_INDIGO} stopOpacity={0.15} />
                <stop offset="100%" stopColor={MORPHO_INDIGO} stopOpacity={0} />
              </linearGradient>
            </defs>
            {sharedGrid}
            {sharedXAxis}
        ...
        <ResponsiveContainer width="100%" height={chartHeight}>
          <AreaChart data={chartData} onClick={handleChartClick}>
            ...
            <Area
              ...
              strokeWidth={2}
              fill="url(#morpho-collateral-grad)"
              dot={false}
              activeDot={{ r: 3 }}
            />
            <Area
      ...
      <div className="flex items-center gap-3 mt-4">
        ...
        <input
          ...
          className="flex-1 h-1 accent-[#4F46E5]"
        />
        <button
